...
def main():
    ...
    '''
    execution_times_str = sys.argv[1:len(sys.argv) // 2 + 1]

    thread_cnt = sys.argv[len(sys.argv) // 2 + 1:-1]

    program_name = sys.argv[-1]

    execution_times = execution_times_from_str_list(execution_times_str)
    execution_time = execution_times[0]
    execution_times_parallel = execution_times[1:]
    print(execution_time)
    print(execution_times_parallel)

    speedups = [execution_time / elem for elem in execution_times_parallel]

    print(speedups)

    plt.figure(1)
    plt.bar(thread_cnt, speedups)
    plt.title(program_name)
    plt.xlabel('number of threads')
    plt.ylabel('speedup')
    plt.grid()
    plt.savefig(program_name + '-speedups-bar.png')
    pass
